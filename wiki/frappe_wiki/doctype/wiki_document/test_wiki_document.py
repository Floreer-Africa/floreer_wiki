...
class TestPrivatePageStaffOnly(WikiDocumentTestBase):
	...
	Upstream now gates reads via the space role config (``check_space_access``),
	which defaults an unconfigured space to "any logged-in user". The patch also
	refuses a logged-in non-System User (e.g. a portal Website User) so an internal
	wiki space can stay published without leaking to webshop customers. Guarded at
	``check_space_access`` because og_image/crawler/print/search all call it directly.
	"""
	...
	def test_system_user_can_view_private_page(self):
		...
		page.check_space_access("read")  # must not raise
