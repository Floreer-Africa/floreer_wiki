...
class WikiDocument(NestedSet):
	...
	def check_space_access(self, ptype="read", user=None):
		...
		# Floreer vendored patch (framework#122), re-applied here after upstream
		# replaced check_guest_access with this space-role model. A PRIVATE page is
		# STAFF-ONLY: refuse any non-System User (e.g. a webshop / portal Website
		# User) so an internal wiki space can stay published — keeping the reading
		# view — without leaking to customers.
		#
		# This MUST live in check_space_access, not in the check_guest_access alias:
		# og_image, crawler_renderer, print, search and the tree/nav paths all call
		# check_space_access directly, so guarding only the alias would leave them
		# open. It is also deliberately independent of the space's role config —
		# can_read_space defaults an unconfigured space to "any logged-in user",
